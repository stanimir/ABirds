	...
	while (!quit &&csdl_setup->GetMainEvent()->type != SDL_QUIT)
	{
		...
		case SDL_MOUSEMOTION:
		{
			if (csdl_setup->GetMainEvent()->motion.state & SDL_BUTTON_LMASK & isButtonDown) {
				...
				physics->bird[0]->SetLinearVelocity(b2Vec2(0, 0));
		...
		case SDL_MOUSEBUTTONUP:
		{
			...
			if (isButtonDown == true) {
				isButtonDown = false;
				b2Vec2 birdLastPos = physics->bird[0]->GetPosition();
				...
				physics->bird[0]->ApplyLinearImpulse(forceToApply, physics->bird[0]->GetWorldCenter(), true);
			}
			//isBirdFlying = false;
			break;
			/*if (isButtonDown == true) {
		...
		if (*(int*)physics->piglet->GetUserData() == 4) {
			physics->piglet->SetActive(false);
		}
		if (physics->piglet->IsActive()) {
			drawWithPhysics(physics->piglet, pigsSprite[0], 0, 0);
		}
